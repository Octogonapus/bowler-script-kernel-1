...
public class RGBColorDetector implements IObjectDetector {
  ...
  public RGBColorDetector(Mat matImage,
                          Scalar rgbMin,
                          Scalar rgbMax,
                          Scalar rgbMin2,
                          Scalar rgbMax2) {
  ...
  public List<Detection> getObjects(BufferedImage in, BufferedImage disp) {
    ...
    Mat sat = lrgb.get(1);
    Mat val = lrgb.get(2);
    Core.subtract(array255, sat, sat);
    Core.subtract(array255, val, val);
    sat.convertTo(sat, CvType.CV_32F);
    val.convertTo(val, CvType.CV_32F);
    Core.magnitude(sat, val, distance);
  ...
  public void setThreshhold(Scalar rgbMin, Scalar rgbMax) {
    this.rgbMin = rgbMin;
    this.rgbMax = rgbMax;
  }

  public void setThreshhold2(Scalar rgbMin2, Scalar rgbMax2) {
    this.rgbMin2 = rgbMin2;
    this.rgbMax2 = rgbMax2;
  }
}
