// How to boxes in case more stuff goes bad http://stackoverflow
// .com/questions/26814069/how-to-set-region-of-interest-in-opencv-java
//				AbstractImageProvider.deepCopy(AbstractImageProvider.matToBufferedImage(Saliency),
// disp);

// ssdasd

//set microsoft life 3000 to approperiate settings or the low sat will cause havoc
//v4l2-ctl --set-ctrl=contrast=10,saturation=200,white_balance_temperature_auto=0,brightness=30
// -d/dev/video1
...
public class SalientDetector implements IObjectDetector {
  ...
  public ArrayList<Rect> SortFindContours(ArrayList<MatOfPoint> a, ArrayList<Rect> b, Mat c, int
      Horizon, int minArea) {
    ...
    if (a.size() != 0 && a.size() < 100) {
      ...
      for (int i = -1; i < FinalSize; i++) {
        ...
        if (i == -1) {
          ...
          test
              = Imgproc.boundingRect(new MatOfPoint(contourFinal.get(0))); // test = rect around
          // the contour's most outer extremes
          area = Imgproc.contourArea(contourFinal.get(0));
        } else {
          test
              = Imgproc.boundingRect(new MatOfPoint(contourFinal.get(i))); // test = rect around
          // the contour's most outer extremes
  ...
  public List<Detection> getObjects(BufferedImage inImg, BufferedImage disp) {
    ...
    if (original.empty()
      ...
      if (stage2 == true) { // lowering
        ...
        Imgproc.findContours(Canny, contourCanny, new Mat(), Imgproc.RETR_EXTERNAL, Imgproc
            .CHAIN_APPROX_NONE);

        STAGE2_BOXES = SortFindContours(contourCanny, STAGE2_BOXES, Canny, Horizon, minArea);

        for (int i = 0; i < STAGE2_BOXES.size(); i++) {
          Core.rectangle(ObjFound, STAGE2_BOXES.get(i).tl(), STAGE2_BOXES.get(i).br(), RedBox, 1,
              8, 0);
        }
        ...
        if (stage1 == false && stage3 == true) { // This is stage 3, it is all solo
          if (prev == false) {
            ...
            Imgproc.findContours(ComparTo, compCont, new Mat(), Imgproc.RETR_EXTERNAL, Imgproc
                .CHAIN_APPROX_NONE);
            ...
            Imgproc.findContours(Saliency, currCont, new Mat(), Imgproc.RETR_EXTERNAL, Imgproc
                .CHAIN_APPROX_NONE);
          ...
          Imgproc.findContours(Saliency, contours, new Mat(), Imgproc.RETR_EXTERNAL, Imgproc
              .CHAIN_APPROX_NONE);

          STAGE1_BOXES = SortFindContours(contours, STAGE1_BOXES, Saliency, Horizon, minArea);

          for (int i = 0; i < STAGE1_BOXES.size(); i++) {
            Core.rectangle(ObjFound, STAGE1_BOXES.get(i).tl(), STAGE1_BOXES.get(i).br(), RedBox,
                1, 8, 0);
          }
          ...
          for (int a1 = 0; a1 < STAGE1_BOXES.size(); a1++) { // process those small
            ...
            ObjectTemp
                = original.submat(aRect).clone();                  // cutout of 100x100 or 250x250
